...
export function JournalEntryCard({ entry, onView, onEdit, onDelete }: JournalEntryCardProps) {
  ...
  const readableDate = formatDateShort(entry.date);
    ...
    <article className="bg-white border border-gray-200 rounded-lg shadow-sm hover:shadow-md transition-shadow p-4 sm:p-6">
      <div className="flex items-start justify-between gap-4 mb-3">
        <div className="flex-1">
          <h3 className="text-sm text-gray-500 font-medium mb-1">{readableDate}</h3>
      ...
      <div className="flex gap-2 justify-end">
        {onView && (
          <button
            ...
            aria-label={`View entry for ${readableDate}`}
        ...
        {onEdit && (
          <button
            ...
            aria-label={`Edit entry for ${readableDate}`}
          ...
          (isConfirmingDelete ? (
            <>
              <button
                ...
                aria-label={`Confirm delete entry for ${readableDate}`}
                className="px-3 py-1 text-sm bg-red-600 text-white rounded hover:bg-red-700 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-red-500"
              >
                Confirm
              </button>
              <button
            ...
            <button
              ...
              aria-label={`Delete entry for ${readableDate}`}
